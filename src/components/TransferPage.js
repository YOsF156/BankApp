...
import { formatNumber, getDateToday, addTransactionToDb } from "./Utils";
...
export const TransferPage = (props) => {
    ...
    const senderSelected = (event) => {
        ...
        users.forEach(user => {
            if (user._id === accountNumber) {
                sender = user;
            }
        })

        const newUsers = users.filter((user, index) => {
            return user._id !== accountNumber;
        });
    ...
    if (!isClient) {
        senders = users.map(user => {
            return (
                <option value={user._id}>{user.fullName} #{user._id}</option>
            )
        });
    }
    ...
    const newReceivers = receivers.map(receiver => {
        if (sender._id !== receiver._id) {
            return (
                <option value={receiver._id}>{receiver.fullName} #{receiver._id}</option>
            )
        }
    ...
    const transferFund = event => {
        ...
        if (sender._id !== 0 && receiver._id !== 0 && receiver._id) {
            // deduct from sender
            let userSender;
                ...
                if (user._id === sender._id) {
                    if (user.balance - amount >= 0) {
                        ...
                        user.transactions.unshift({
                            title: `Fund transfer to ${receiver.fullName} #${receiver._id}`,
                            amount: amount,
                            type: "debit",
                            date: getDateToday()
                        });
                        addTransactionToDb(user._id, user.balance, user.transactions)
                        senderSuccess = true;
                        userSender = user;
                        console.log("🚀 ~ file: TransferPage.js:95 ~ transferFund ~ userSender", userSender)
            ...
            // add to receiver 
            if (senderSuccess) {
                console.log("🚀 ~ file: TransferPage.js:101 ~ transferFund ~ senderSuccess", senderSuccess)
                users.forEach(async user => {
                    if (user._id === receiver._id) {
                        user.balance += amount;

                        user.transactions.unshift({
                            title: `Fund transfer from ${userSender.fullName} #${userSender._id}`,
                            amount: amount,
                            type: "credit",
                            date: getDateToday()
                        });
                        let res = await addTransactionToDb(user._id, user.balance, user.transactions)
    ...
    if (isClient) {
        senderField = <input type="text" name="sender" value={`${client.fullName} #${client._id}`} disabled />
    }
    ...
    return (
        <section id="main-content">
            <form id="form" onSubmit={transferFund}>
                ...
                <select value={receiver._id || 0} onChange={receiverSelected} name="receiver">
